use crate::markdown::renderer::{
    AlertKind, ColumnAlignment, InlineSpan, MarkdownBlock, MarkdownDocument, TableBlock,
};
...
fn render_spans_with_width<'a, Message: 'static + Clone>(
    ...
    fn push_span_to_iced<'a>(
        span: &'a InlineSpan,
        base_size: f32,
        default_color: Color,
        theme: &'a EditorTheme,
        is_dimmed: bool,
        link_override_color: Option<Color>,
        out: &mut Vec<Span<'a, ()>>,
    ) {
        let active_color = link_override_color.unwrap_or(default_color);
        match span {
            InlineSpan::Text(s) => {
                out.push(Span::new(s.as_str()).size(base_size).color(active_color));
            }
            InlineSpan::Bold(s) => {
                out.push(
                    Span::new(s.as_str())
            ...
            InlineSpan::Link { children, url } => {
                ...
                if children.is_empty() {
                    out.push(Span::new(url.as_str()).size(base_size).color(link_color));
                } else {
...
fn measure_span_width(span: &InlineSpan, base_size: f32, font_name: &str) -> f32 {
    match span {
        InlineSpan::Text(s) | InlineSpan::Italic(s) | InlineSpan::Strikethrough(s) => s
            .chars()
            .map(|ch| crate::ui::canvas_editor::measure_glyph_advance(ch, base_size, font_name))
            .sum(),
        InlineSpan::Bold(s) => {
            // Bold glyphs in proportional or variable fonts have a slightly wider advance
            s.chars()
                .map(|ch| {
                    let adv =
                        crate::ui::canvas_editor::measure_glyph_advance(ch, base_size, font_name);
                    if (' '..='~').contains(&ch) {
                        adv * 1.05
                    } else {
                        adv
        ...
        InlineSpan::Code(s) => {
            let text_w: f32 = s
                ...
                .sum();
            text_w + 12.0 // 5px left + 5px right padding + 2px borders
        }
        InlineSpan::Styled { text: s, style } => {
            let bold_factor = if style.bold { 1.05 } else { 1.0 };
            s.chars()
                .map(|ch| {
                    let adv =
                        crate::ui::canvas_editor::measure_glyph_advance(ch, base_size, font_name);
        ...
        InlineSpan::Link { children, url } => {
            if children.is_empty() {
                ...
                children
                    .iter()
                    .map(|child| measure_span_width(child, base_size, font_name))
                    .sum()
            }
        }
        InlineSpan::ImageFallback { alt, url } => {
...
pub fn measure_spans_width(spans: &[InlineSpan], base_size: f32, font_name: &str) -> f32 {
    spans
        .iter()
        .map(|s| measure_span_width(s, base_size, font_name))
        .sum()
}

fn render_table_block<'a, Message: 'static + Clone>(
    table: &'a TableBlock,
    theme: &'a EditorTheme,
    font_name: &str,
) -> Element<'a, Message> {
    ...
    let mut col_widths: Vec<f32> = vec![60.0; num_cols];
    for (c, header) in table.headers.iter().enumerate() {
        let cell_w = measure_spans_width(header, 12.5, font_name);
        col_widths[c] = col_widths[c].max(cell_w + 24.0);
    }
    for row_data in &table.rows {
        for (c, cell) in row_data.iter().enumerate() {
            if c < num_cols {
                let cell_w = measure_spans_width(cell, 12.0, font_name);
                col_widths[c] = col_widths[c].max(cell_w + 24.0);
            }
        }
    }
    for w in &mut col_widths {
        *w = w.clamp(60.0, 420.0);
    }
    ...
    for (c, width) in col_widths.iter().enumerate() {
        let header_spans = table.headers.get(c).map(|s| s.as_slice()).unwrap_or(&[]);
        let align = table
            .alignments
            .get(c)
            .copied()
    ...
    for (r, row_data) in table.rows.iter().enumerate() {
        ...
        for (c, width) in col_widths.iter().enumerate() {
            let cell_spans = row_data.get(c).map(|s| s.as_slice()).unwrap_or(&[]);
            let align = table
                .alignments
                .get(c)
                .copied()
            ...
            let cell_txt =
                render_spans_with_width(cell_spans, 12.0, theme.config.fg, theme, Length::Shrink);
                ...
                .class(cosmic::theme::Container::Custom(Box::new(move |_| {
                    container::Style {
                        background: if is_even_row {
                            Some(cosmic::iced::Color::from_rgba(1.0, 1.0, 1.0, 0.04).into())
                        } else {
                            None
                        },
                        border: cosmic::iced::Border {
    ...
    let table_scroll = scrollable(table_col)
...
fn render_single_block<'a, Message: 'static + Clone>(
    ...
    match block {
        ...
        MarkdownBlock::Paragraph(spans) => render_spans(spans, 13.5, theme.config.fg, theme),
        MarkdownBlock::CodeBlock { lang, code } => {
            let header = text(if lang.is_empty() {
                "text"
            } else {
                lang.as_str()
            })
        ...
        MarkdownBlock::ListItem {
            ...
            let bullet_widget = text(bullet_str).class(cosmic::theme::Text::Color(bullet_color));
            ...
            if children.is_empty() {
                ...
                let mut item_col = column::with_capacity(children.len() + 1).spacing(8);
                item_col = item_col.push(list_row);
                let child_indent = indent + 18.0;
                for child in children {
                    let child_widget = render_single_block(child, theme, font_name);
                    let child_row = row::with_capacity(2)
                        .spacing(0)
                        .push(Space::new().width(Length::Fixed(child_indent)))
                        .push(child_widget)
                        .width(Length::Fill);
                    item_col = item_col.push(child_row);
                }
                item_col.into()
            }
        }
        MarkdownBlock::BlockQuote(inner_blocks) => {
            ...
            let left_bar = container(Space::new().width(Length::Fixed(3.5)).height(Length::Fill))
                .class(cosmic::theme::Container::Custom(Box::new(move |_| {
                    container::Style {
            ...
            let mut inner_col = column::with_capacity(inner_blocks.len()).spacing(8);
            for b in inner_blocks {
                inner_col = inner_col.push(render_single_block(b, theme, font_name));
            }

            let content_box = container(inner_col).padding([2, 8]).width(Length::Fill);
        ...
        MarkdownBlock::Alert { kind, spans } => {
            ...
            let left_bar = container(Space::new().width(Length::Fixed(4.0)).height(Length::Fill))
                .class(cosmic::theme::Container::Custom(Box::new(move |_| {
                    container::Style {
        ...
        MarkdownBlock::Table(table) => render_table_block(table, theme, font_name),
...
pub fn view_markdown<'a, Message: 'static + Clone>(
    ...
    for block in &doc.blocks {
        col = col.push(render_single_block(block, theme, font_name));
    }

    let scroll = scrollable(col).width(Length::Fill).height(Length::Fill);
