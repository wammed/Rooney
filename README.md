...
- 🌳 **True Background Tree-sitter, Dual Generation-Tracked Markdown & Search**: Heavy Tree-sitter parsing for massive files (> 2MB–50MB) and Markdown preview document parsing (on toggle, edit, or spec changes across all active and inactive tabs) are offloaded to background worker tasks (`tokio::task::spawn_blocking`), completely eliminating UI thread freezing. All asynchronous search and Markdown operations feature strict single-generation tracking (`search_generation` and `markdown_generation`), guaranteeing that stale results from in-flight workers during rapid typing, spec switching, or preview toggling are cleanly and safely discarded. Keeps keystroke latency down to **~3.9 µs** in release mode (< 0.1 ms in debug) even on 50MB buffers.
