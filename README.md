...
- 🌳 **True Background Tree-sitter, Markdown & Async Generation-Tracked Search**: Heavy Tree-sitter parsing for massive files (> 2MB–50MB) and Markdown document preview parsing (on toggle, edit, or spec changes) are offloaded to background worker tasks (`tokio::task::spawn_blocking`), completely eliminating UI thread freezing. During search on large files (> 2MB), search query input immediately spawns a background worker with generation-based stale result discard, while document body editing with active search uses 100ms idle debouncing with generation advancement to eliminate full-text line scans during continuous typing, keeping keystroke latency down to **~3.9 µs** in release mode (< 0.1 ms in debug) even on 50MB buffers.
