...
- 🌳 **True Background Tree-sitter & Debounced Large-File Search**: Heavy Tree-sitter parsing for massive files (> 2MB–50MB) is offloaded to background worker tasks (`tokio::task::spawn_blocking`), completely eliminating UI thread freezing. Active search query line scans on files > 2MB are automatically debounced during continuous typing, keeping typing latency down to **~3.9 µs** in release mode (< 0.1 ms in debug) even on 50MB buffers with active search open.
