...
- 🪟 **Dual-Pane Split Editing & Text-Focused GFM Live Preview**: Side-by-side editing (`Ctrl + \`), independent multi-tabs, synchronized editing, and live Markdown preview (`Ctrl + M`) with dynamic specification switching between **GFM** (GitHub Flavored Markdown) and standard **CommonMark**.
  - **Text-Focused GFM Compliant**: A pure native Rust renderer engineered without bloated browser engines (WebView) or heavy image decoding pipelines. Delivers complete coverage of GFM tables, alerts (Callouts: `[!NOTE]`, `[!TIP]`, `[!IMPORTANT]`, `[!WARNING]`, `[!CAUTION]`), task lists, inline formatting (bold, italic, inline code, strikethrough), hyperlinks, footnotes, and strict `breaks: false` adherence.
  - **Lightweight Image-Free Design**: Image tags (`![alt](url)` / `<img>`) safely and elegantly fall back to lightweight inline badges (`󰋩 [Image: alt]`). By eliminating image rasterization and external network overhead, Rooney guarantees 144+ FPS butter-smooth scrolling and minimal memory consumption as a fast, accurate GFM editor.
