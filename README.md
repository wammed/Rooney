...
- 🌳 **Hybrid Tree-sitter AST & 50MB Ultra-Low Latency Typing**: Instant synchronous delta parsing for files ≤ 2MB (< 4ms), paired with 100ms debounced delayed parsing for massive files (> 2MB–50MB), delivering **0.028 ms** typing latency on 50MB / 1.85M-line files alongside per-line syntax highlight caching.
...
- 🇯🇵 **Pixel-Accurate Japanese IME & Real Glyph Metrics**: Full Wayland text-input protocol support (Fcitx5 / Mozc / IBus), live pre-edit underline preview, and direct `cosmic-text` shaped glyph metrics caching with `RwLock` shared concurrency and `clear_glyph_cache` (resolving East Asian Ambiguous characters like `――` U+2015, emojis, and combining characters) preventing cursor drift.
