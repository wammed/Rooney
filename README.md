...
- 🪟 **Dual-Pane Split Editing & GFM-Oriented Markdown Live Preview**: Side-by-side editing (`Ctrl + \`), independent multi-tabs, synchronized editing, and live Markdown preview (`Ctrl + M`) with dynamic specification switching between **GFM** (GitHub Flavored Markdown) and standard **CommonMark**.
  - **Text-Focused GFM Support (Zero Remote I/O)**: Pure native Rust renderer engineered without bloated browser engines (WebView) or external network fetching. Delivers rich and accurate coverage of GFM tables, alerts (Callouts: `[!NOTE]`, `[!TIP]`, `[!IMPORTANT]`, `[!WARNING]`, `[!CAUTION]`), nested task lists, composite inline formatting (nested bold, italic, strikethrough, inline code), display-only styled links/autolinks, footnotes, and strict `breaks: false` adherence.
  - **Intentional Image-Free Design & Safe HTML Fallback**: Images are intentionally not loaded or rendered; image syntax (`![alt](url)` / `<img>`) is represented using a lightweight offline text/badge fallback (`󰋩 [Image: alt]`). Rooney does not execute JavaScript, render full browser-like HTML, or make remote HTTP/HTTPS requests, guaranteeing zero external network attack surface, minimal memory footprint, and 144+ FPS butter-smooth scrolling.
