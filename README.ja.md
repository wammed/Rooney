...
- 🇯🇵 **サブピクセル高精度な日本語 IME 完全対応 & 実グリフメトリクス連携**: Wayland text-input プロトコル（Fcitx5 / Mozc / IBus）をネイティブサポート。インライン変換下線プレビューに加え、`cosmic-text` 実グリフレイアウト計測キャッシュ（U+2015 水平バー等の Ambiguous/CJK 記号完全一致）により、マルチバイト文字入力・選択・カーソル位置ズレを根本解消。
