...
- 🪟 **2分割ペイン編集 & Text-Focused GFM リアルタイムプレビュー**: ツールバーやショートカット（`Ctrl + \`）で即座に左右分割。独立マルチタブ、同期編集、設定画面から **GFM**（GitHub Flavored Markdown）と標準 **CommonMark** を動的に切り替え可能（`Ctrl + M`）。
  - **Text-Focused GFM Compliant（テキスト特化型 GFM 準拠）**: 重厚なブラウザエンジン（WebView）プロセスや画像デコード処理を完全に排除した純粋なネイティブ Rust レンダラーを採用。GFM テーブル、アラート（Callout: `[!NOTE]` / `[!TIP]` / `[!IMPORTANT]` / `[!WARNING]` / `[!CAUTION]`）、タスクリスト、インライン装飾（太字・斜体・インラインコード・打ち消し線）、ハイパーリンク、脚注、および `breaks: false` を完全網羅。
  - **画像なし軽量設計**: 画像構文（`![alt](url)` / `<img>`）は洗練されたインラインバッジ（`󰋩 [画像: alt]`）へスマートにフォールバック。重い画像ラスター展開や外部ネットワーク通信による描画遅延・メモリ肥大化をゼロに抑え、144+ FPS の超軽快なスクロールと極小メモリ消費を両立する「軽量で正確な GFM エディタ」を実現。
...
- **リソース保護 & 低フットプリント**: 50MB を超える巨大ファイルの誤オープン防止、1MB の AI ストリーミング受信バッファ制限、有界 $O(1)$ リングバッファ Undo/Redo スタック（`VecDeque`）、毎フレームの不要な一時アロケーションを抑えたイテレータ直接描画。さらに Markdown プレビューでは重厚な画像デコーダや WebView を排除した Text-Focused 設計により、外部画像の取得やラスター展開に伴うメモリ枯渇・クラッシュリスクを根本排除。
