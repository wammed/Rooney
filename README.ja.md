...
- 🌳 **真の非同期バックグラウンド Tree-sitter & Markdown プレビュー & 世代管理非同期検索**: 巨大ファイル（> 2MB〜50MB）の重い Tree-sitter AST 解析および Markdown プレビューパース（表示切替・本文編集・仕様切替時）をワーカースレッド（`tokio::task::spawn_blocking`）へ完全移譲し、UI スレッドの描画フリーズ（144+ FPS）を完全に排除。大容量ファイル（> 2MB）の検索機能は、クエリ入力時に即時バックグラウンドワーカーを起動し世代管理（generation-based）で古い検索結果を破棄、本文編集時は100msアイドルデバウンス＋世代番号インクリメントにより入力中の全文走査を完全排除し、検索実行中であってもキーストロークレイテンシをリリースビルドで **約 3.9 µs**（デバッグ時でも < 0.1 ms）に維持。
