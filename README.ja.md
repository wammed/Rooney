...
<p align="center">
  <a href="README.md">English</a> | <a href="README.ja.md">日本語</a> | <a href="LICENSES.ja.md">ライセンス通知</a>
</p>
...
- 🪟 **2分割ペイン編集 & GFM 志向の Markdown リアルタイムプレビュー**: ツールバーやショートカット（`Ctrl + \`）で即座に左右分割。独立マルチタブ、同期編集、設定画面から **GFM**（GitHub Flavored Markdown）と標準 **CommonMark** を動的に切り替え可能（`Ctrl + M`）。
  - **GFM-oriented Markdown support（テキスト特化型 Markdown レンダリング）**: 重厚なブラウザエンジン（WebView）、CSS レイアウトエンジン、JavaScript を排除した純粋なネイティブ Rust レンダラーを採用。GFM テーブル（実グリフメトリクス幅計算および横スクロール対応）、アラート（Callout: `[!NOTE]` / `[!TIP]` / `[!IMPORTANT]` / `[!WARNING]` / `[!CAUTION]`）、タスクリスト、複合インライン装飾（入れ子を含む太字・斜体・インラインコード・打ち消し線）、表示専用の装飾リンク／自動リンク（意図的設計としてクリック不可・ブラウザ起動なし）、脚注、および `breaks: false` を正確に解釈。リスト項目は depth（階層深さ）と文書順序によって自然な階層構造を保持し、コードブロック等の入れ子ブロックは各 ListItem の children に構造化して保持。
  - **外部アクセス禁止ポリシー（Zero Remote I/O）**: **Markdown rendering performs zero remote I/O.** Markdown レンダリング処理はリモート画像の取得、スクリプト実行、ブラウザ起動、外部 HTTP/ネットワークアクセスを一切行いません。（※Rooney 全体としては、ローカル AI 機能のために PC 内の `localhost:11434` の Ollama デーモンと通信を行いますが、外部クラウドとの通信は行いません）。
  - **画像なし軽量設計 & 安全な HTML フォールバック**: 画像は安全なテキスト／バッジフォールバックとして描画：画像構文（`![alt](url)` / `<img>`）は洗練されたインラインバッジ（`󰋩 [画像: alt]`）へスマートにフォールバック。HTML は `<br>` の改行変換など安全なテキストフォールバックとして処理され、ブラウザ相当のフル HTML 描画は行いません。
...
- **リソース保護 & Zero Remote I/O Markdown**: 50MB を超える巨大ファイルの誤オープン防止、1MB の AI ストリーミング受信バッファ制限、有界 $O(1)$ リングバッファ Undo/Redo スタック（`VecDeque`）、毎フレームの不要な一時アロケーションを抑えたイテレータ直接描画。さらに **Markdown rendering performs zero remote I/O** の方針のもと、重厚な画像デコーダや WebView を排除し、外部アクセスのリスクやメモリ肥大化を根本排除。
...
- 📝 **リッチなインライン Markdown AST 描画**: エディタキャンバス内で太字・斜体・インラインコード・装飾リンクなどの構造をインライン直接描画。
