...
- 📜 **ピースツリー Rope バッファ**: `ropey` を採用し、メモリの連続再配置を抑えた効率的な文字・行編集を提供。
- 🌳 **インプロセス Tree-sitter 構文ハイライト**: 12以上の言語（Rust, Python, JS, TS, C, C++, Bash, Fish, JSON, TOML, YAML, Markdown）を高精度かつ低負荷でリアルタイムハイライト。マルチバイト文字（日本語・全角記号・絵文字）環境下でも UTF-8 バイトオフセットと文字インデックスを厳密変換。
...
- 🧭 **快適な自動追従スクロール & 独立した右端スクロールバー**: カーソルが画面上端・下端に達した際に視界外へ隠れないようマージンを保って追従。2分割時も含め各ペイン右端に独立したスクロールバー（プロポーショナルつまみ、ドラッグ移動、トラッククリックジャンプ）を完備。
- 🇯🇵 **サブピクセル高精度な日本語 IME 完全対応**: Wayland text-input プロトコル（Fcitx5 / Mozc / IBus）をネイティブサポート。インライン変換下線プレビューと、半角・全角実寸幅の厳密計算により、文字入力時のカーソル位置ズレを解消。
...
- 🏎️ **ビューポート仮想スクロール（Viewport-Based Virtualization）**: 画面内の可視行のみにレイアウト計算と描画を限定し、巨大ファイルでの表示・スクロール速度を飛躍的に高速化。
- 📐 **実グリフメトリクス（`cosmic-text`）連携**: 近似計算ではなく、`cosmic-text` が算出する実際のグリフレイアウト幅を直接参照し、プロポーショナルフォントや CJK 文字幅の計算精度をさらに洗練。
- ⚡ **Tree-sitter 増分ハイライト・行キャッシュ**: AST 差分解析と行単位のトークンキャッシュにより、1行の編集時に発生する構文再解析負荷を極小化。
