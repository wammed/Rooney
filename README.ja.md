...
- 🌳 **真の非同期バックグラウンド Tree-sitter & 世代管理 Markdown/検索パイプライン**: 巨大ファイル（> 2MB〜50MB）の重い Tree-sitter AST 解析および Markdown プレビューパース（表示切替・本文編集・仕様切替時、非アクティブタブを含む全タブ）をワーカースレッド（`tokio::task::spawn_blocking`）へ完全移譲し、UI スレッドの描画フリーズ（144+ FPS）を完全に排除。検索機能と Markdown プレビューの双方が厳密な単一世代管理アーキテクチャ（`search_generation` および `markdown_generation`）で保護され、高速タイピング、仕様変更、プレビュー開閉が競合しても古いスナップショットの解析結果を安全に破棄（Stale Protection 100%）。検索実行中であってもキーストロークレイテンシをリリースビルドで **約 3.9 µs**（デバッグ時でも < 0.1 ms）に維持。
