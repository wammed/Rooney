...
- 🌳 **真の非同期バックグラウンド Tree-sitter & 検索デバウンス**: 巨大ファイル（> 2MB〜50MB）の重い Tree-sitter AST 解析をワーカースレッド（`tokio::task::spawn_blocking`）へ完全移譲し、UI スレッドの描画フリーズ（144+ FPS）を完全に排除。さらに検索クエリ実行中における 50MB 全文走査をタイピング中は自動デバウンスし、検索実行中であってもキーストロークレイテンシをリリースビルドで **約 3.9 µs**（デバッグ時でも < 0.1 ms）に維持。
