...
- 🏎️ **ビューポート仮想スクロール描画**: 画面内の可視行（+ 上下マージン数行）のみにレイアウト計算を限定。50MB / 185万行の巨大ファイルでもサブミリ秒（< 0.45 ms）でレイアウトを完了し、144+ FPS の超軽快な描画・スクロールを提供。
- 🌳 **Tree-sitter ハイブリッド解析 & 50MB 超低遅延タイピング**: 文字の挿入・削除・置換時に `InputEdit` を厳密伝達し、通常ファイル（≤ 2MB）では同期即時インクリメンタル更新（< 4ms）、巨大ファイル（> 2MB〜50MB）では 100ms デバウンス遅延解析を採用。50MB / 185万行でも **0.028 ms** の超低遅延タイピングと、論理行ハイライトキャッシュによる 144+ FPS レンダリングを両立。
...
- 🇯🇵 **サブピクセル高精度な日本語 IME 完全対応 & 実グリフメトリクス連携**: Wayland text-input プロトコル（Fcitx5 / Mozc / IBus）をネイティブサポート。インライン変換下線プレビューに加え、`cosmic-text` 実グリフレイアウト計測キャッシュ（`RwLock` 共有ロック化・`clear_glyph_cache` 完備、U+2015 水平バー等の Ambiguous/CJK/絵文字/結合文字完全一致）により、マルチバイト文字入力・選択・カーソル位置ズレを根本解消。
