...
- 🎨 **全20種の洗練された Classic & Neon テーマ**: Tokyo Night、Catppuccin、Gruvbox、Synthwave '84、Cyberpunk Neon などを網羅。ウィンドウ透明度・背景ディミングとともに `󰒓 Aesthetics` モーダルで一元設定可能。
...
| `Ctrl + D` | **現在行の複製** |
| `Ctrl + ,` | **Aesthetics 設定**を開く（テーマ、フォント、サイズ、AI） |
...
- **機密ファイル自動シールド**: `.env*`、秘密鍵（`id_rsa`、`id_ed25519`、`credentials`、`*.pem`、`*.key`）の編集中は、AI 自動補完リクエストの送信を自動的にバイパスして情報漏洩を遮断。
- **原子的ファイル保存（Atomic Write）**: 同一階層の一時ファイル（`.{filename}.tmp.{pid}`）への書き込み・同期（`sync_all`）・置換（`rename`）により、OS クラッシュ時の 0 バイト破損を根絶。
- **パストラバーサル & 境界保護**: ファイル・ディレクトリ作成やリネーム時のディレクトリ脱出（`..`、`/`）をサニタイズ。ルート（`/`）やホームディレクトリの誤った再帰削除をセーフガードで防止。
- **リソース保護ガードレール**: 50MB を超える巨大ファイルの誤オープン防止、1MB の AI ストリーミング受信バッファ制限、ファイルツリーの再帰深度上限ガードを完備。
